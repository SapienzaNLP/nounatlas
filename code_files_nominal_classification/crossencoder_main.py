...
from pipeline_utils import negative_embedding_finder_data_construction
from models.CrossEncoderClassifier import CrossEncoderClassifier as ModelClassifier

from datasets.DynamicParametrizedDataModule import DynamicDataModule, DynamicDataset
import data_preprocessing.wordnet_utils as wnutils
...
if __name__ == "__main__" and len(sys.argv) > 1:
    ...
    parser.add_argument(
        ...
        default="train",
        required=True,
        choices=["finetune_dataset", "finetune_model", "train", "valid", "test", "predict_test", "predict"],
...
dataset_hyperparameters_tuning = {
    ...
    "binary_dataset" : [True],
    "use_nominal_bn_definitions": [False, True],
    "use_verbal_bn_definitions": [False],
    "clean_definitions": [True, False],
    "use_lemmas" : [True, False],
    "max_num_samples" : [-1],
    "batch_size" : [32],
...
model_hyperparameters_tuning = {
    ...
    # Model hyperparams:
    "transformer_name" : ["sentence-transformers/all-MiniLM-L12-v2", "MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli", "sentence-transformers/all-mpnet-base-v2", "roberta-base"],
...
dataset_hyperparameters = {
    ...
    # "custom_data_construction_fn" : negative_embedding_finder_data_construction,
}
...
model_hyperparameters = {
    "num_classes" : len(wnutils.inventory.va_dict.keys()),
    # Model hyperparams:
    "transformer_name" : "sentence-transformers/all-mpnet-base-v2", # "MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli",
    "fine_tune_transformer" : True,
    "mlp_num_hidden" : None,
    "dropout" : 0.2,
    "custom_sep_token": dataset_hyperparameters["sentence_separator"] is not None,
    # Training hyperparams:
    "learning_rate" : 3e-5,
    "adam_eps" : 1e-8,
    "adam_weight_decay" : 0.0,
...
# region predict (on test)
    ...
    data_module = generate_dataset_type(dataset_hyperparameters_x, **generate_dataset_hyperparameters)
    ...
    for nominal_synset_name, nominal_synset_object in pbar:
        for nominal_definition in nominal_synset_object["definitions"][:prediction_hyperparameters["max_num_nominal_definitions"]]:
            ...
            for frame_name, frame_defs_list in verbal_dict.items():
                ...
                if dataset_hyperparameters["sentence_separator"] is not None:
                    samples = [f"{nominal_definition} {dataset_hyperparameters['sentence_separator']} {vdef}" for vdef in verbal_definitions]
                else:
